...
class ParserHooks implements ParserFirstCallInitHook {
	...
	private const TILES = [
		...
		// Special:MyPage redirects to the viewer's own user page (their grid),
		// so the tile is per-user yet safe to cache in the shared dashboard.
		'mytime' => [ 'Special:MyPage', '⏱️' ],
	];
	 ...
	 */
	public function renderGrid( Parser $parser, string $scope = '', string $id = '', string $arg = '', string $name = '' ): array {
		$out = $parser->getOutput();
		$out->updateCacheExpiry( 0 );
		...
		// gridAddMap/gridFixed take the page-supplied customer ($arg) and name so a
		// just-created entity's row works before SMW has indexed it.
		return $this->html( ( $page ? $this->weekNav( $page, $days ) : '' ) . $this->table->renderGrid(
			...
			$gridUser, $this->gridAddMap( $scope, $id, $arg, $name, $names ),
			$this->gridFixed( $scope, $id, $arg, $name, $names ) ) );
	}
	 ...
	 */
	private function gridFixed( string $scope, string $id, string $arg, string $name, array $names ): array {
		$cust = static fn ( string $c ): array => [ $c, $names['customers'][$c] ?? $c ];
		// Prefer the page-supplied name (fresh) over the SMW-aggregate one.
		$job = static fn ( string $j, string $n = '' ): array =>
			[ $j, $n !== '' ? $n : ( $names['jobs'][$j] ?? $j ) ];
		switch ( $scope ) {
			case 'customer':
				return [ 'customer' => $cust( $id ) ];
			case 'job':
				// The job page carries its customer ($arg) and name; use them.
				return [ 'customer' => $cust( $arg ), 'job' => $job( $id, $name ) ];
			case 'task':
				return [
					'customer' => $cust( $this->query->jobCustomer( $arg ) ),
					'job' => $job( $arg ),
					'task' => [ $id, $name !== '' ? $name : ( $names['tasks'][$id] ?? $id ) ],
				];
			default:
				return [];
		}
	 ...
	 */
	private function gridAddMap( string $scope, string $id, string $arg, string $name, array $names ): array {
		$map = $this->query->customerJobsMap();
		switch ( $scope ) {
			case 'customer':
				return isset( $map[$id] ) ? [ $id => $map[$id] ] : [];
			case 'job':
				// $arg is the customer the job page carries. Synthesize the job so
				// its General bucket is always addable even before SMW has indexed
				// the new job; its tasks come from SMW when they exist.
				if ( $arg === '' ) {
					return [];
				}
				$entry = $map[$arg] ?? [ 'name' => $names['customers'][$arg] ?? $arg, 'jobs' => [] ];
				$jobs = array_values( array_filter( $entry['jobs'] ?? [],
					static fn ( $j ) => $j['id'] === $id ) );
				if ( !$jobs ) {
					$jobs = [ [ 'id' => $id, 'name' => $name !== '' ? $name : ( $names['jobs'][$id] ?? $id ),
						'tasks' => [] ] ];
				}
				$entry['jobs'] = $jobs;
				return [ $arg => $entry ];
	...
	/**
	 * {{#timetracker_progress:<jobId>|<estimate>}} — the job's logged time
	 * against its estimate (a bar + "X of Y (Z%)"). With no estimate it renders
	 * nothing: the grid's own footer total already covers the job's time.
	 *
	...
	public function renderProgress( Parser $parser, string $jobId = '', string $estimateArg = '' ): array {
		...
		// read from SMW, so an estimate edit shows in the bar immediately.
