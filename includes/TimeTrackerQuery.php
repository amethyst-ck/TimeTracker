...
class TimeTrackerQuery {
	...
	public function customerJobsMap( string $keepJobId = '', string $keepTaskId = '' ): array {
		...
		foreach ( $tasksByJob as &$taskList ) {
			usort( $taskList, static fn ( $a, $b ) => strnatcasecmp( $a['name'], $b['name'] ) );
		}
		unset( $taskList );
		...
		// Natural, case-insensitive order everywhere the picker/add-row list them.
		foreach ( $map as &$entry ) {
			usort( $entry['jobs'], static fn ( $a, $b ) => strnatcasecmp( $a['name'], $b['name'] ) );
		}
		unset( $entry );
		uasort( $map, static fn ( $a, $b ) => strnatcasecmp( $a['name'], $b['name'] ) );
		return $map;
	}
