	...
	public function execute( $subPage ) {
		...
		if ( $request->wasPosted() && $csrf->matchToken( $request->getVal( 'wpEditToken' ) ) ) {
			...
			if ( $request->getVal( 'action' ) === 'start' ) {
				...
				// No-JS fallback: the grid can't update in place, so redirect with
				// a saved total and a one-shot refresh once SMW has committed. With
				// JS the Stop button is intercepted (see ext.timetracker.timer) and
				// this path is not taken.
				$result = $this->timer->stop( $this->getAuthority() );
				if ( $result !== null && $result['total'] > 0 ) {
					$query['saved'] = (string)$result['total'];
					$query['ttfresh'] = '1';
				}
			}
