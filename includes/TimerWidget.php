...
class TimerWidget {
	...
	private function renderRunning( array $running, ?Title $returnTo, bool $showContext = true ): string {
		...
		// On a job/task page the surrounding page fixes the customer/job/task (no
		// context rows); elsewhere it's the free picker. The AJAX stop handler
		// reads these to re-render the matching idle widget in place.
		return Html::rawElement( 'div', [
			'class' => 'tt-card tt-running',
			'data-tt-surface' => $showContext ? 'picker' : 'job',
			'data-tt-job' => $running['job'],
			'data-tt-task' => $running['task'],
		], $clock . $rows . $stop );
	}

	/**
	 * The idle Start widget for a surface, re-rendered so an AJAX stop can swap it
	 * for the running card without a reload. 'job' = a job/task page's fixed Start
	 * button; anything else = the free customer/job/task picker.
	 */
	public function renderIdle(
		IContextSource $context, string $surface, string $job, string $task, ?Title $returnTo
	): string {
		$this->context = $context;
		return $surface === 'job'
			? $this->renderJobStart( $context, $job, $task, $returnTo )
			: $this->renderPickerStart( $returnTo );
	}
	...
	public function renderJobStart( IContextSource $context, string $jobId, string $taskId, ?Title $returnTo ): string {
		...
		// tt-start centers the lone Start button (no picker on a scoped page).
		return Html::rawElement( 'div', [ 'class' => 'tt-card tt-start' ], $form );
	}
